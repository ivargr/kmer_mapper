...
class Sequences:
    def __init__(self, all_sequences, intervals):
        self.sequences = all_sequences
        self.offsets = intervals[0][1:]
...
class OneLineFastaParser(TextParser):
    ...
    def get_sequences(self, array):
        ...
        # print(new_intervals)
        # print(array[mask])
        return Sequences(array[mask], new_intervals)
...
class KmerHash:
    ...
    def __init__(self, k=31):
        self.k=k
        self.POWER_ARRAY = np.power(4, np.arange(k))
        self.REV_POWER_ARRAY = np.power(4, np.arange(k)[::-1])
